...
class Backend_Articles_Form extends AVO_Data_Form
{
				...
				new AVO_Layout_Control(array(
					...
					new AVO_Layout_Control(array(
						...
						new Backend_Image_Upload_Field_Set(array(
							'title'					=> 'Portrait teaser image',
							'note'					=> 'This image will be used when blog post is teased in portrait mode on start page, and if you have chosen to show teaser as portrait in categorypages.',
							'model'					=> 'Model_Article',
					...
					new AVO_Layout_Control(array(
						...
						new Backend_Image_Upload_Field_Set(array(
							'title'					=> 'Landscape teaser image and top blog post image',
							'note'					=> 'This image will be used when blog post is teased in landscape mode on start page, and if you have chosen to show teaser as landscape in category pages',
							'model'					=> 'Model_Article_Landscape_Image',
	...
	public function on_validate(AVO_Data_Form_Action_Result $result)
	{
		...
		$url_title_occupied = DB::query(Database::SELECT, 'SELECT COUNT(*) FROM articles WHERE (:id IS NULL OR id <> :id) AND url_title = :url_title')
